...
const NeonButton = memo<NeonButtonProps>(
  ({
    ...
    border-2 border-purple-500 rounded-full overflow-hidden neon-btn
  ...
  `.trim();
    ...
    const content = (
      <>
        ...
        {/* Subtle soft glow */}
        <div className="absolute inset-0 rounded-full blur-sm bg-purple-500 opacity-20 hover:opacity-40 transition-opacity duration-300"></div>

        {/* Button text */}
        <span className="relative z-10 whitespace-nowrap">{children}</span>

        {/* Scoped glow animation only for NeonButton */}
        <style>{`
        @keyframes neon-glow {
          0%, 100% {
            box-shadow:
